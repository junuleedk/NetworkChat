...
public class MultiServer {
	...
	public static void main(String[] args) {
		//멤버변수 : 소켓관련, IO스트림 관련, String 인스턴스
		...
		try {
			/*
			 9999번으로 포트번호를 설정한 후 서버인스턴스를 생성하고
			 클라이언트의 접속을 기다린다.(Scanner 인스턴스와 같이 대기 상태가 된다.)
			 */
			...
			/*
			  ...
			  : 원격지 측 클라이언트와 IP주소와 Port 번호를 얻어온다.
			 
			  getLocalAddress() / getLocalPort()
			  :연결이 시작된 네티워크 즉 서버측의 정보를 얻어온다.
			  
			 */
			System.out.println(socket.getInetAddress() + 
					"(클라이언트)의 " + socket.getPort() + "포트를 통해 " + 
