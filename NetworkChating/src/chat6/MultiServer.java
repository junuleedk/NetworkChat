...
public class MultiServer {
   ...
   public MultiServer() {
	   ...
	   /*HashMap 동기화 설정. 쓰레드가 사용자 정보에 동시접근하는 것을
	    차단한다.*/
   ...
   public void init() {
      ...
      try {
         ...
         /*1명의 클라이언트가 접속될 때마다 허용해주고 동시에 쓰레드를 생성한다.*/
         while(true) {
   ...
   /*접속된 모든 클라이어튼 측으로 부터 서버의 메세지를 Echo 해주는 역할을 수행한다.
   (이전 단계에서는 보낸 사람에게만 Echo 되었다.)*/
   public void sendAllMsg(String name, String msg) {
	   
	   /*Map에 저장된 클라이언트의 key를 얻어온다. key에는 대화명이 저장되어있다.*/
	   Iterator<String> it = clientMap.keySet().iterator();
	   
	   //앞에서 얻어 온 대화명(key값)의 갯수만큼 반복한다.
	   while(it.hasNext()) {
		   try {
			   ...
			   /*
			    클라이언트에게 메세지를 전달할 때 매개변수로 name이 
			    있는 경우와 없는 경우를 구분해서 전달한다.
			    */
			   if(name.equals("")) {
				   /*입장 혹은 퇴장에서 사용되는 부분*/
				   it_out.println(msg);
   ...
   class MultiServerT extends Thread {
      ...
      public void run() {
         ...
         try {
	         ...
	         //두번째 입력부터는 메세지이므로 모든 클라이언트에게 Echo한다. 
            while(in != null) {
               s = in.readLine();
               if(s == null) 
            	   break;
            ...
            System.out.println("예외:"+ e);
        	 ...
        	 /*
        	  클라이언트가 접속을 종료하면 Socket예외가 발생하게 되어 finally절로 진입
        	  하게 된다. 이때 '대화명'을 통해 Map에서 클라이언트의 정보를 제거한다.
        	  */
